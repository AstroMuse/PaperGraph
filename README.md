...
Here are some areas where community contributions or future iterations might focus:
...
### Prerequisites
- Node.js (v18+)
- Redis Server (Running on default port `6379`. Windows users can run via WSL `redis-server` or install Memurai. If the port is different, you need to modify the IP and port in server.js)
...
### 环境要求
- Node.js (v18+)
- Redis Server (默认端口 `6379` 运行。Windows 用户可通过 WSL 运行 `redis-server` 或安装 Memurai, 若端口不一样则需去server.js修改IP和端口)
